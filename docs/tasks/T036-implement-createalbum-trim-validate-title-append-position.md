---
id: T036
title: "Implement createAlbum (trim/validate title, append position)"
...
Implement `createAlbum` (trim/validate title, append `position`) in `lib/db/repository.ts` (maps to spec story US2)
...
- Source task: `T036` in `specs/001-photo-album-organizer/tasks.md`.
- Depends on: None.
- Blocks: T046.
