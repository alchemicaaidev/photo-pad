---
id: T033
title: "Unit test exif (reads DateTimeOriginal; falls back when absent)"
...
Unit test `exif` (reads DateTimeOriginal; falls back when absent) in `lib/images/exif.test.ts` (maps to spec story US2)
...
- Source task: `T033` in `specs/001-photo-album-organizer/tasks.md`.
- Depends on: None.
- Blocks: None.
