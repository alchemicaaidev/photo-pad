---
id: T021
title: "Implement PhotoTile (thumbnail <img> with intrinsic width/height to avoid layout shift; button with accessible name)"
...
Implement `PhotoTile` (thumbnail `<img>` with intrinsic width/height to avoid layout shift; button with accessible name) in `components/photos/PhotoTile.tsx` (maps to spec story US1)
...
- Source task: `T021` in `specs/001-photo-album-organizer/tasks.md`.
- Depends on: None.
- Blocks: T023.
