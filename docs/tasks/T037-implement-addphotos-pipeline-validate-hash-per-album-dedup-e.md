---
id: T037
title: "Implement addPhotos pipeline (validate → hash → per-album dedup → EXIF date → thumbnail → persist; returns AddResult outcomes; recompute derivedDate + photoCount)"
milestone: "Phase 4: US2 Create Albums & Add Photos"
priority: 3
estimate: 3
blockedBy: [T028, T030, T032, T034]
blocks: [T047]
areas:
  - storage
parent: null
...
Implement `addPhotos` pipeline (validate → hash → per-album dedup → EXIF date → thumbnail → persist; returns `AddResult` outcomes; recompute `derivedDate` + `photoCount`) in `lib/db/repository.ts` (depends on T028, T030, T032, T034) (maps to spec story US2)
...
- Source task: `T037` in `specs/001-photo-album-organizer/tasks.md`.
...
- Dependencies (T028, T030, T032, T034) are complete or in review.
