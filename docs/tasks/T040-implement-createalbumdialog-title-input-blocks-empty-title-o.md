---
id: T040
title: "Implement CreateAlbumDialog (title input; blocks empty title) on Dialog"
...
Implement `CreateAlbumDialog` (title input; blocks empty title) on `Dialog` in `components/albums/CreateAlbumDialog.tsx` (maps to spec story US2)
...
- `components/albums/CreateAlbumDialog.tsx`
...
- Source task: `T040` in `specs/001-photo-album-organizer/tasks.md`.
- Depends on: None.
- Blocks: T046.
