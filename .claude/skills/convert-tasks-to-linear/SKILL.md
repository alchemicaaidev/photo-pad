...
description: >-
  Validate, preview, or publish a docs/tasks/task-package.yaml planning wave to
  Linear with milestone assignments, parent/sub-issue relationships, blocker
  relations, area labels, and publish state. Use after create-implementation-plan
  produces a task package that should be projected into Linear.
---
...
```bash
uv run --script .claude/skills/convert-tasks-to-linear/scripts/convert_tasks_to_linear.py \
  validate \
...
This converter shells out to the `linear` skill's GraphQL helper at
`.claude/skills/linear/scripts/linear_graphql.py`, so that skill must be present
and `LINEAR_API_KEY` must be set for `apply`.
